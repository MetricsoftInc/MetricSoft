    ...
    public class QualityIssue
    {
        ...
        public int MailNotify(string mailURL)
        {
            ...
            try
            {
                ...
                if (this.IssueOccur.QS_ACTIVITY == "CST")
                {   // customer incident:  assume responsible locations are primary company and b2b location is customer plant
                    partNum = part.DRAWING_REF;
                    notifyList = SQMModelMgr.SelectNotifyHierarchy(SessionManager.PrimaryCompany().COMPANY_ID, (decimal)this.IssueOccur.INCIDENT.RESP_BUS_ORG_ID, (decimal)this.IssueOccur.INCIDENT.RESP_PLANT_ID, (decimal)this.IssueOccur.INCIDENT.DETECT_PLANT_ID, TaskRecordType.CustomerQualityIncident);
                }
                else
                {   // creceiving incident:  assume detected locations are primary company and b2b location is supplier plant
                    partNum = part.PART_NUM;
                    notifyList = SQMModelMgr.SelectNotifyHierarchy(SessionManager.PrimaryCompany().COMPANY_ID, (decimal)this.IssueOccur.INCIDENT.DETECT_BUS_ORG_ID, (decimal)this.IssueOccur.INCIDENT.DETECT_PLANT_ID, (decimal)this.IssueOccur.INCIDENT.RESP_PLANT_ID, TaskRecordType.SupplierQualityIncident);
                }
                HashSet<string> emailNotifylList = SQMModelMgr.DistinctNotifyEmailList(notifyList);

                string subject = "";
                string body = "<br /><br />";
                string title = "";

                subject = sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "SUBJECT").Select(s => s.XLAT_LONG).FirstOrDefault();
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "ISSUE").Select(s => s.XLAT_LONG).FirstOrDefault() + WebSiteCommon.FormatID(this.Incident.INCIDENT_ID, 6);
                body += "<br />";
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "STATUS").Select(s => s.XLAT_LONG).FirstOrDefault() + WebSiteCommon.GetXlatValue("recordStatus", this.IssueOccur.STATUS);
                body += "<br /><br />";
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "PLANT").Select(s => s.XLAT_LONG).FirstOrDefault() + SQMModelMgr.LookupPlant((decimal)this.IssueOccur.INCIDENT.DETECT_PLANT_ID).PLANT_NAME;
                body += "<br />";
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "CREATEBY").Select(s => s.XLAT_LONG).FirstOrDefault() + (createPerson == null ? this.Incident.CREATE_BY : createPerson.FIRST_NAME + " " + createPerson.LAST_NAME);
                body += "<br />";
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "PART").Select(s => s.XLAT_LONG).FirstOrDefault() + partNum + "  " + part.PART_NAME;
                body += "<br />";
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "PROBLEM").Select(s => s.XLAT_LONG).FirstOrDefault() + WebSiteCommon.GetXlatValueLong("problemArea", this.IssueOccur.PROBLEM_AREA);
                if (!string.IsNullOrEmpty(this.IssueOccur.SEVERITY))
                {
                    body += ("  (" + WebSiteCommon.GetXlatValueLong("incidentSeverity", this.IssueOccur.SEVERITY) + ")");
                }
                body += "<br />";
                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "DESCR").Select(s => s.XLAT_LONG).FirstOrDefault() + this.IssueOccur.INCIDENT.DESCRIPTION;
                body += "<br /><br />";

                body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "RESPONSIBLE").Select(s => s.XLAT_LONG).FirstOrDefault() + SQMModelMgr.LookupPlant((decimal)this.IssueOccur.INCIDENT.RESP_PLANT_ID).PLANT_NAME;
                if (!string.IsNullOrEmpty(this.IssueOccur.RELATED_INCIDENTS))
                {
                    body += "<br />";
                    body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "RELATED").Select(s => s.XLAT_LONG).FirstOrDefault() + this.IssueOccur.RELATED_INCIDENTS;
                }

                if (this.IssueOccur.EST_ACT_COST.HasValue && this.IssueOccur.EST_ACT_COST > 0)
                {
                    body += "<br />";
                    body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "ACTCOST").Select(s => s.XLAT_LONG).FirstOrDefault() + SQMBasePage.FormatValue((decimal)this.IssueOccur.EST_ACT_COST, 2);
                    body += ("&nbsp;(" + this.IssueOccur.CURRENCY_CODE + ")&nbsp;&nbsp;&nbsp;");
                    body += this.IssueOccur.ACT_COST_COMMENT;
                }
                if (this.IssueOccur.EST_POT_COST.HasValue && this.IssueOccur.EST_POT_COST > 0)
                {
                    body += "<br />";
                    body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "POTCOST").Select(s => s.XLAT_LONG).FirstOrDefault() + SQMBasePage.FormatValue((decimal)this.IssueOccur.EST_POT_COST, 2);
                    body += ("&nbsp;(" + this.IssueOccur.CURRENCY_CODE + ")&nbsp;&nbsp;&nbsp;");
                    body += this.IssueOccur.POT_COST_COMMENT;
                }

                if (this.IssueOccur.PROBCASE_REQD == true)
                {
                    body += "<br />";
                    body += sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "8D").Select(s => s.XLAT_LONG).FirstOrDefault();
                }
               
                body += "<br /><br />";
                body += sets.Where(s => s.SETTING_GROUP == "NOTIFY" && s.SETTING_CD == "URL").Select(s => s.XLAT_LONG).FirstOrDefault();

                string url = (mailURL + "Quality/Quality_Issue.aspx?c=" + this.IssueOccur.QS_ACTIVITY + "&i=" + EncryptionManager.Encrypt(this.Incident.INCIDENT_ID.ToString()));
                body += url;
                
                body += "<br /><br />";
                body += sets.Where(s => s.SETTING_GROUP == "NOTIFY" && s.SETTING_CD == "NOREPLY").Select(s => s.XLAT_LONG).FirstOrDefault() + "</b>";

                // send mail to issue assignees ...
                foreach (TASK_STATUS task in this.TeamTask.TaskList)
                {
                    bool sendMail = this.StatusChanged;
                    if ((this.ResponseRequired  || this.ResponsibleChanged) && (task.TASK_TYPE == "R" &&  task.COMPLETE_DT.HasValue == false))
                        sendMail = true;
                    if (this.DetectedChanged && (task.TASK_TYPE == "C" && task.COMPLETE_DT.HasValue == false))
                        sendMail = true;

                    if (sendMail) 
                    {
                        PERSON person = SQMModelMgr.LookupPerson((decimal)task.RESPONSIBLE_ID, "");
                        if (person != null)
                        {
                            if (this.StatusChanged)
                            {
                                title = "<b>" + sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "CHANGE").Select(s => s.XLAT_LONG).FirstOrDefault() + "</b>";
                            }
                            else 
                            {
                                title = "<b>" + sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "TASK").Select(s => s.XLAT_LONG).FirstOrDefault() + "</b>";
                                title += "<br /><br />";
                                title += (sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "TASK").Select(s => s.XLAT_SHORT).FirstOrDefault() + " <b>" + WebSiteCommon.GetXlatValueLong("taskType", task.TASK_TYPE) + "</b>");
                            }
                            Thread thread = new Thread(() => WebSiteCommon.SendEmail(person.EMAIL, subject, title + body, createPerson == null ? "" : createPerson.EMAIL, "web", this.AttachmentsList));
                ...
                // end mail to notify persons 
                foreach (string email in emailNotifylList)
                {
                    bool sendMail = this.StatusChanged;
                    ...
                    if (sendMail)
                    {
                        if (this.NotifyRequired)
                        {
                            title = "<b>" + sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "NOTIFY").Select(s => s.XLAT_LONG).FirstOrDefault() + "</b>";
                        }
                        else
                        {
                            title = "<b>" + sets.Where(s => s.SETTING_GROUP == group && s.SETTING_CD == "CHANGE").Select(s => s.XLAT_LONG).FirstOrDefault() + "</b>";
                        }
                        Thread thread = new Thread(() => WebSiteCommon.SendEmail(email, subject, title + body, createPerson == null ? "" : createPerson.EMAIL, "web", new List<ATTACHMENT>()));
                        thread.IsBackground = true;
                        thread.Start();
                        ++status;
                    }
