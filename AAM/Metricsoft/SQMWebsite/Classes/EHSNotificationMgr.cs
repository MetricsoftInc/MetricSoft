...
namespace SQM.Website
{
	public static class EHSNotificationMgr
	{
        ...
        #region AddApproveremail
        public static void NotifyAddApprover(decimal? emailApproverTo,decimal? personid, string emailSubject)
        {
            ...
            PSsqmEntities entities = new PSsqmEntities(); 
                ...
                if (string.IsNullOrEmpty(appUrl))
                    appUrl = "the website";
            PERSON person = (from p in entities.PERSON where p.PERSON_ID == personid select p).FirstOrDefault();
            PERSON Approver = (from p in entities.PERSON where p.PERSON_ID == emailApproverTo select p).FirstOrDefault();
            List < SETTINGS > mailSettings = SQMSettings.SelectSettingsGroup("MAIL", "");

                PERSON createBy = new PERSON();
                try
                {
                    createBy = null;//SQMModelMgr.LookupPersonByEmpID(entities, theTask.CREATE_ID.ToString());
                }
                catch { }
            ...
            string actionText = "";// XLATList.Where(x => x.XLAT_GROUP == "NOTIFY_SCOPE_TASK" && x.XLAT_CODE == scopeAction).FirstOrDefault().DESCRIPTION;
                ...
                string emailBody = //XLATList.Where(x => x.XLAT_GROUP == "INCIDENT_NOTIFY" && x.XLAT_CODE == theTask.STATUS).FirstOrDefault().DESCRIPTION + "<br/>" +
                                ...
                                "Approver Created" + "<br/>" +
                                "<br/>" +
                                "Approver Created Test" + "<br/>" +
                                "<br/>" + 
                                "<br/>" +
                                Resources.LocalizedText.CreatedBy + ": " + createByName + "<br/>" +
                                "<br/>"  ;

                  WebSiteCommon.SendEmail(Approver.EMAIL, emailSubject, emailBody, "", "web", null, mailSettings); 
                WriteEmailLog(entities, Approver.EMAIL, mailSettings.Find(x => x.SETTING_CD == "MailFrom").VALUE, emailSubject, emailBody,Convert.ToInt32(person.PERSON_ID),Approver.PERSON_ID, "assigned approver inforamtion", "rtn", "");
             
        }
        public static void NotifyApprover(INCIDENT incident,decimal? emailApproverTo, string incidentStep, string scopeAction, List<string> infoList)
        {
            ...
            string emailTo = Approver.EMAIL;
            string subject = XLATList.Where(x => x.XLAT_GROUP == "INCIDENT_NOTIFY" && x.XLAT_CODE == scopeAction).FirstOrDefault().DESCRIPTION_SHORT.Replace("[0]", infoList.ElementAt(0));
            string emailSubject = subject + " (" + plant.PLANT_NAME + ")";
            string body = XLATList.Where(x => x.XLAT_GROUP == "INCIDENT_NOTIFY" && x.XLAT_CODE == scopeAction).FirstOrDefault().DESCRIPTION.Replace("[1]", infoList.ElementAt(0));
            string emailBody = body + "<br/>" +
                            "<br/>" +
                            XLATList.Where(x => x.XLAT_GROUP == "INCIDENT_STEP" && x.XLAT_CODE == incidentStep).FirstOrDefault().DESCRIPTION +
                            "<br/><br/>" +
